...
- **`server/icons.py`** — one on-brand `Icon` (gold background, dark-brown glyph — Nyuchi's own Mzizi ecosystem mineral, distinct from Bundu's copper) per tool *domain*, not per tool; passed to every `@_shared.mcp.tool(icons=[...])` call and to `MCPServer(icons=[icons.SERVER])`. `tests/test_icons.py` fails if a tool is ever added without one.
...
Everything on `ynab.nyuchi.com` other than `/mcp*` and `/health` (i.e. `/` and `/privacy-policy`) is a static Astro site, `worker/site`, built to `worker/site/dist` and served directly by Cloudflare via the `assets` binding in `wrangler.jsonc` — that request never reaches `worker/src/index.ts`'s `fetch` handler at all, so the container is never woken just to serve the landing page or privacy policy.
